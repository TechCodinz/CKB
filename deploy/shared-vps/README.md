...
This profile runs the public Rust CKB MCP/Reality service under `/opt/platform/ckb/core` without touching LeanTrader or the public 80/443 edge.

## Host contract

- LeanTrader remains untouched at `/opt/leantraderbot` under systemd.
...
Then verify the API container can resolve/reach `ckb-mcp:3000` across `ckb_mesh` before any Vercel cutover.
...
Rollback is CKB-local. Do not restart or reconfigure LeanTrader during a CKB rollback.
