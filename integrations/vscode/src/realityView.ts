...
import type { RuntimeRealityFeed } from './runtimeReality';
    ...
    resolveWebviewView(view: vscode.WebviewView) {
        ...
        view.webview.onDidReceiveMessage(async message => {
            switch (message?.type) {
                ...
                case 'runtimeRefresh': await vscode.commands.executeCommand('ckb.refreshRuntimeReality'); break;
...
<script nonce="${token}">
const vscode=acquireVsCodeApi();const payload=${jsonForScript(payload)};const state=payload.state||{};const runtime=payload.runtime||{};const activity=state.activity||state.bundle?.activity||{};const dna=state.dna||state.bundle?.dna||{};const hotspots=Array.isArray(activity.hotspots)?activity.hotspots:[];const runtimeNodes=hotspots.filter(n=>n&&n.runtimeObserved);const faults=hotspots.filter(n=>Number(n?.errorRate||n?.runtime?.errorRate||0)>0);let lens='semantic',depth='system',intent='auto',flowFilter='all',traceCursor=0;
