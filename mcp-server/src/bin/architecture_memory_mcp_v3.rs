use ckb_core::{
    ArchitectureMemoryEngine, CausalArchitectureEngine, DependencyGraph, FileAnalysis,
    LanguageParser, NodeId,
};
...
/// v3 stores the graph as opaque bincode bytes. DependencyGraph intentionally
/// does not implement Clone, so this format lets CKB persist a borrowed graph
/// without weakening the core type merely to satisfy the MCP process.
...
/// Backward-compatible reader for memories written by the previous MCP server.
#[derive(Serialize, Deserialize)]
...
fn stable_key(input: &str) -> String {
    // Deterministic FNV-1a key/fingerprint. It is not used for cryptographic
    // integrity, credentials, authentication, or source-code secrecy.
...
/// Fingerprint both Git HEAD and the current working tree. This closes the
/// important v1 freshness gap where unsaved-to-Git edits could leave memory
/// looking fresh merely because HEAD had not changed.
    ...
    if let Some(status) = status {
        ...
        for line in status.lines() {
            ...
            let path = raw_path
                ...
                .unwrap_or(raw_path)
                .trim_matches('"');
            let object_hash = run_git(root, &["hash-object", "--", path])
...
fn memory_base_dir() -> PathBuf {
    ...
    if let Ok(home) = std::env::var("HOME").or_else(|_| std::env::var("USERPROFILE")) {
        if !home.trim().is_empty() {
            return PathBuf::from(home).join(".ckb").join("memory");
        }
    }
...
fn persist_memory(root: &str, graph: &DependencyGraph) -> anyhow::Result<PersistMeta> {
    let canonical = canonical_root(root);
    let saved_at = chrono::Utc::now().to_rfc3339();
    let current_head = git_head(&canonical);
    let current_workspace = workspace_fingerprint(&canonical);
    ...
    let persisted = PersistedMemoryV3 {
        ...
        git_head: current_head.clone(),
        workspace_fingerprint: current_workspace.clone(),
        graph_bytes,
    };
...
fn load_memory(root: &str) -> anyhow::Result<LoadedMemory> {
    let path = memory_path(root);
    let bytes = std::fs::read(&path).map_err(|e| {
        anyhow::anyhow!(
            "No durable CKB memory found at {}: {}",
            path.display(),
            e
        )
    })?;
    ...
    if let Ok(v3) = bincode::deserialize::<PersistedMemoryV3>(&bytes) {
        if v3.version == "ckb-durable-memory-v3" {
            let graph = bincode::deserialize::<DependencyGraph>(&v3.graph_bytes)?;
            return Ok(LoadedMemory {
                root: v3.root,
                saved_at: v3.saved_at,
                git_head: v3.git_head,
                workspace_fingerprint: v3.workspace_fingerprint,
                graph,
                source_version: v3.version,
            });
        }
    }
    ...
    if let Ok(v1) = bincode::deserialize::<PersistedMemoryV1>(&bytes) {
        if v1.version == "ckb-durable-memory-v1" {
            return Ok(LoadedMemory {
                ...
                graph: v1.graph,
                source_version: "ckb-durable-memory-v1".into(),
            });
        }
    }

    anyhow::bail!(
        "Unsupported or corrupt CKB architecture memory at {}",
        path.display()
    )
}
...
fn freshness(
    ...
    let state = match (
        ...
        (Some(saved), Some(now), Some(saved_ws), Some(now_ws))
            if saved == now && saved_ws == now_ws =>
        {
            "fresh"
        }
...
fn architecture_delta(before: &DependencyGraph, after: &DependencyGraph) -> ArchitectureDelta {
    let old_nodes: HashSet<String> = before
        .nodes()
        .into_iter()
        .map(|node| node.id.0.clone())
        .collect();
    let new_nodes: HashSet<String> = after
        .nodes()
        .into_iter()
        .map(|node| node.id.0.clone())
        .collect();
    let old_edges: HashSet<String> = before
        .edges()
        .into_iter()
        .map(edge_identity)
        .collect();
    let new_edges: HashSet<String> = after
        .edges()
        .into_iter()
        .map(edge_identity)
        .collect();
    ...
    ArchitectureDelta {
        ...
        from_nodes: old_nodes.len(),
        to_nodes: new_nodes.len(),
        from_edges: old_edges.len(),
        to_edges: new_edges.len(),
        added_nodes,
        removed_nodes,
        added_edges,
        removed_edges,
        ...
        "tools": [
            {
                ...
                "description":"Build and durably persist evidence-backed architecture memory for a repository.",
                "inputSchema":{"type":"object","properties":{"path":{"type":"string"}},"required":["path"]}
            },
            {
                "name":"ckb_memory_resume",
                "description":"Resume durable architecture memory across model/MCP restarts and verify Git plus working-tree freshness.",
                "inputSchema":{"type":"object","properties":{"path":{"type":"string"}},"required":["path"]}
            },
            {
                "name":"ckb_memory_refresh",
                "description":"Refresh loaded memory from current source, persist it, and return the exact architecture delta since the prior memory state.",
                "inputSchema":{"type":"object","properties":{"force":{"type":"boolean","default":false}}}
            },
            {
                "name":"ckb_memory_status",
                "description":"Report loaded memory, persistence, Git HEAD and working-tree freshness.",
                "inputSchema":{"type":"object","properties":{}}
            },
            {
                "name":"ckb_memory_delta",
                "description":"Return the most recent real architecture delta produced by ckb_memory_refresh.",
                "inputSchema":{"type":"object","properties":{}}
            },
            {
                "name":"ckb_memory_query",
                "description":"Retrieve a bounded evidence-backed architecture neighborhood for a question, symbol or path.",
                "inputSchema":{"type":"object","properties":{"query":{"type":"string"},"depth":{"type":"integer","minimum":0,"maximum":5,"default":2},"limit":{"type":"integer","minimum":1,"maximum":100,"default":12}},"required":["query"]}
            },
            {
                "name":"ckb_symbol_memory",
                "description":"Retrieve bounded software memory centered on one symbol/path before editing it.",
                "inputSchema":{"type":"object","properties":{"symbol":{"type":"string"},"depth":{"type":"integer","minimum":0,"maximum":5,"default":2},"limit":{"type":"integer","minimum":1,"maximum":100,"default":8}},"required":["symbol"]}
            },
            {
                "name":"ckb_context_capsule",
                "description":"Create compact model-ready architecture context with freshness evidence and a strict character budget.",
                "inputSchema":{"type":"object","properties":{"query":{"type":"string"},"depth":{"type":"integer","minimum":0,"maximum":5,"default":2},"limit":{"type":"integer","minimum":1,"maximum":100,"default":12},"max_chars":{"type":"integer","minimum":1000,"maximum":60000,"default":12000}},"required":["query"]}
            },
            {
                "name":"ckb_code_dna",
                "description":"Return explainable Code DNA health/risk metrics from graph topology, cycles and runtime observations.",
                "inputSchema":{"type":"object","properties":{}}
            },
            {
                "name":"ckb_causal_path",
                "description":"Explain the shortest proven directed architecture path between exact CKB node IDs.",
                "inputSchema":{"type":"object","properties":{"source":{"type":"string"},"target":{"type":"string"},"max_depth":{"type":"integer","minimum":1,"maximum":32,"default":12}},"required":["source","target"]}
            },
            {
                "name":"ckb_failure_cone",
                "description":"Return the real transitive upstream dependent cone for a symbol without pretending a runtime failure occurred.",
                "inputSchema":{"type":"object","properties":{"root":{"type":"string"},"max_depth":{"type":"integer","minimum":1,"maximum":32,"default":12}},"required":["root"]}
            }
        ]
    })
}
    ...
    match name {
                ...
                Ok((graph, files)) => match persist_memory(path, &graph) {
                    Ok(meta) => {
                        let response = json!({
                            ...
                            "workspaceFingerprint":meta.workspace_fingerprint,
                            "memoryFile":memory_path(path).to_string_lossy(),
                            "freshness":"fresh",
                            "memoryLifetime":"durable across MCP/model restarts",
                            "synthetic":false
                        });
                        let mut state = session.write().await;
                        state.graph = Some(graph);
                        state.root = Some(meta.root);
                        state.git_head = meta.git_head;
                        state.workspace_fingerprint = meta.workspace_fingerprint;
                        state.saved_at = Some(meta.saved_at);
                        state.last_delta = None;
                        text_result(response)
                    }
                    Err(e) => error_result(format!(
                        "Architecture scan succeeded but durable persistence failed: {e}"
                    )),
                },
        ...
        "ckb_memory_resume" => {
            ...
            match load_memory(path) {
                Ok(loaded) => {
                    let (fresh, current_head, current_workspace) = freshness(
                        &loaded.root,
                        loaded.git_head.as_deref(),
                        loaded.workspace_fingerprint.as_deref(),
                    );
                    let response = json!({
                        "status":"resumed",
                        "path":loaded.root,
                        ...
                        "savedGitHead":loaded.git_head,
                        "currentGitHead":current_head,
                        "savedWorkspaceFingerprint":loaded.workspace_fingerprint,
                        "currentWorkspaceFingerprint":current_workspace,
                        "freshness":fresh,
                        "sourceMemoryVersion":loaded.source_version,
                        "mustRefreshBeforeHighConfidenceEdits":fresh != "fresh",
                        "memoryFile":memory_path(path).to_string_lossy(),
                        "synthetic":false
                    });
                    let mut state = session.write().await;
                    state.graph = Some(loaded.graph);
                    state.root = Some(loaded.root);
                    state.git_head = loaded.git_head;
                    state.workspace_fingerprint = loaded.workspace_fingerprint;
                    state.saved_at = Some(loaded.saved_at);
                    state.last_delta = None;
                    text_result(response)
                }
        ...
        "ckb_memory_refresh" => {
            ...
            // Move the old graph out of the session. DependencyGraph is
            // deliberately non-Clone, and the stdio MCP loop processes calls
            // serially, so taking ownership here is both safe and compile-safe.
            let (root, old_graph, saved_head, saved_workspace) = {
                let mut state = session.write().await;
                let Some(root) = state.root.clone() else {
                    return error_result("No architecture memory loaded. Resume or scan first.");
                };
                let Some(graph) = state.graph.take() else {
                    return error_result("No architecture graph loaded.");
                };
                (
                    root,
                    graph,
                    state.git_head.clone(),
                    state.workspace_fingerprint.clone(),
                )
            };

            let (fresh, _, _) = freshness(
                &root,
                saved_head.as_deref(),
                saved_workspace.as_deref(),
            );
            if fresh == "fresh" && !force {
                let nodes = old_graph.node_count();
                let edges = old_graph.edge_count();
                session.write().await.graph = Some(old_graph);
            ...
            match build_graph(&root).await {
                Ok((new_graph, files)) => {
                    ...
                    match persist_memory(&root, &new_graph) {
                        Ok(meta) => {
                            ...
                            let mut state = session.write().await;
                            state.graph = Some(new_graph);
                            state.root = Some(meta.root);
                            state.git_head = meta.git_head;
                            state.workspace_fingerprint = meta.workspace_fingerprint;
                            state.saved_at = Some(meta.saved_at);
                            state.last_delta = Some(delta_value);
                            text_result(response)
                        }
                        Err(e) => {
                            session.write().await.graph = Some(old_graph);
                            error_result(format!(
                                "Refreshed graph but durable persistence failed; prior memory was restored: {e}"
                            ))
                        }
                    }
                }
                Err(e) => {
                    session.write().await.graph = Some(old_graph);
                    error_result(format!("CKB memory refresh failed; prior memory was restored: {e}"))
                }
            }
        }
        "ckb_memory_status" => {
            ...
            let Some(graph) = state.graph.as_ref() else {
                return text_result(json!({
                    "loaded":false,
                    "message":"No architecture memory is loaded. Use ckb_memory_resume or ckb_memory_scan.",
            ...
            let (fresh, current_head, current_workspace) = freshness(
                root,
                state.git_head.as_deref(),
                state.workspace_fingerprint.as_deref(),
            );
            text_result(json!({
                "loaded":true,
                "path":root,
                ...
                "currentGitHead":current_head,
                "savedWorkspaceFingerprint":state.workspace_fingerprint,
                "currentWorkspaceFingerprint":current_workspace,
                "freshness":fresh,
                "mustRefreshBeforeHighConfidenceEdits":fresh != "fresh",
        ...
        "ckb_memory_delta" => {
            let state = session.read().await;
            text_result(state.last_delta.clone().unwrap_or_else(|| {
                json!({
                    "available":false,
                    "message":"No refresh delta exists in this MCP session yet.",
                    "synthetic":false
                })
            }))
        }
        "ckb_memory_query" | "ckb_symbol_memory" | "ckb_context_capsule" => {
            ...
            let query_key = if name == "ckb_symbol_memory" {
                "symbol"
            } else {
                "query"
            };
            ...
            let limit = args
                ...
                .unwrap_or(if name == "ckb_symbol_memory" { 8 } else { 12 })
                as usize;
            ...
            match ArchitectureMemoryEngine::query(graph, query, depth, limit) {
                Ok(slice) if name == "ckb_context_capsule" => {
                    ...
                    text_result(json!({
                        "version":"ckb-context-capsule-v1",
                        "query":query,
                        "freshness":fresh,
                        "currentGitHead":current_head,
                        "currentWorkspaceFingerprint":current_workspace,
                        "mustRefreshBeforeHighConfidenceEdits":fresh != "fresh",
                        "rootIds":slice.root_ids,
                        "retrievedNodes":slice.nodes.len(),
                        "retrievedEdges":slice.edges.len(),
                        "context":context,
                        "characterBudget":max_chars,
                        "evidencePolicy":slice.evidence_policy,
                        "synthetic":false
                    }))
                }
        ...
        "ckb_causal_path" => {
            ...
            let Some(source) = args.get("source").and_then(Value::as_str) else {
                return error_result("source is required");
            };
            ...
            match CausalArchitectureEngine::shortest_path(
                graph,
                &NodeId(source.into()),
                &NodeId(target.into()),
                max_depth,
        ...
        "ckb_failure_cone" => {
            ...
            let Some(root) = args.get("root").and_then(Value::as_str) else {
                return error_result("root is required");
            };
            let max_depth = args
                .get("max_depth")
                .and_then(Value::as_u64)
                .unwrap_or(12) as usize;
            match CausalArchitectureEngine::failure_cone(
                graph,
                &NodeId(root.into()),
                max_depth,
            ) {
...
async fn handle(session: &Arc<RwLock<MemorySession>>, request: Value) -> Value {
    ...
    let result = match method {
        "initialize" => json!({
            ...
            "capabilities":{"tools":{}},
            "serverInfo":{"name":"ckb-architecture-memory","version":"3.0.1"},
