...
async fn authorize_tool(
    ...
    let internal_secret = state.internal_secret.as_ref().ok_or_else(|| {
        "CKB OAuth introspection is not configured on this deployment.".to_string()
    })?;
    ...
    match name {
        ...
        "ckb_generate_ai_rules" => {
            ...
            upstream(
                state,
                Method::GET,
                format!("/api/v1/rules?project_id={}", encode_component(&project_id)),
                None,
            )
            .await
        }
...
mod tests {
    ...
    /// Every tool the gateway advertises must be scope-gated. A tool that
    /// `required_scope` does not recognise would be unreachable, and a tool
    /// missing from `tools()` could never be discovered.
    #[test]
    fn every_advertised_tool_declares_a_known_scope() {
        let advertised = tools();
        assert_eq!(advertised.len(), 13, "canonical registry size changed");
        for tool in &advertised {
            let name = tool["name"].as_str().expect("tool name");
            let scope = required_scope(name)
                .unwrap_or_else(|| panic!("{name} is advertised but has no required scope"));
            assert!(
                scope == ARCHITECTURE_SCOPE || scope == REPOSITORY_SCAN_SCOPE,
                "{name} declares an unknown scope {scope}"
            );
        }
    }
    ...
    fn tool_names_are_unique() {
        let advertised = tools();
        let mut names: Vec<&str> = advertised
            .iter()
        ...
        assert_eq!(
            names.len(),
            total,
            "duplicate tool name in canonical registry"
        );
    }

    /// Repository scanning is the only capability that reaches outside CKB's
    /// own evidence, so it must be the only tool requiring `repository:scan`.
    #[test]
    fn only_repository_scanning_requires_the_scan_scope() {
        let advertised = tools();
        let scan_scoped: Vec<&str> = advertised
            .iter()
            .map(|tool| tool["name"].as_str().expect("tool name"))
            .filter(|name| required_scope(name) == Some(REPOSITORY_SCAN_SCOPE))
            .collect();
        assert_eq!(scan_scoped, vec!["ckb_scan_repository"]);
    }

    #[test]
    fn unknown_tools_have_no_scope() {
        assert_eq!(required_scope("ckb_delete_everything"), None);
        assert_eq!(required_scope(""), None);
    }

    /// Each tool advertises an OAuth security scheme carrying its own scope
    /// plus offline_access, so a client can see what linking will require
    /// before it attempts a call.
    #[test]
    fn every_tool_advertises_its_oauth_security_scheme() {
        for tool in tools() {
            let name = tool["name"].as_str().expect("tool name");
            let schemes = tool["securitySchemes"]
                .as_array()
                .unwrap_or_else(|| panic!("{name} has no securitySchemes"));
            assert_eq!(schemes.len(), 1, "{name} should declare exactly one scheme");
            assert_eq!(schemes[0]["type"], "oauth2", "{name} must use oauth2");
            let scopes: Vec<&str> = schemes[0]["scopes"]
                .as_array()
                .expect("scopes array")
                .iter()
                ...
                .collect();
            assert!(
                scopes.contains(&required_scope(name).expect("known scope")),
                "{name} scheme omits its required scope"
            );
            assert!(
                scopes.contains(&OFFLINE_SCOPE),
                "{name} scheme omits offline_access"
            );
        }
    }

    #[test]
    fn every_tool_exposes_an_object_input_schema() {
        for tool in tools() {
            let name = tool["name"].as_str().expect("tool name");
            assert_eq!(tool["inputSchema"]["type"], "object", "{name} input schema");
            assert_eq!(
    ...
    /// Exposed repository operations are read-only. If a tool ever stops
    /// declaring that, the annotation contract has been broken.
    #[test]
    fn no_tool_claims_to_modify_the_target_repository() {
        for tool in tools() {
            let name = tool["name"].as_str().expect("tool name");
            assert_eq!(
                tool["annotations"]["readOnlyHint"], true,
                "{name} must remain read-only against the target repository"
            );
        }
    }

    /// OAuth callers are namespaced by subject so two users scanning the same
    /// logical project never observe each other's Reality data.
    #[test]
    fn oauth_callers_are_isolated_per_user() {
        let alice = AuthContext {
            user_id: Some("user-alice".into()),
        };
        let bob = AuthContext {
            user_id: Some("user-bob".into()),
        };
        let alice_key = scoped_project_id(&alice, "acme-api");
        let bob_key = scoped_project_id(&bob, "acme-api");

        assert_eq!(alice_key, "chatgpt:user-alice:acme-api");
        assert_ne!(
            alice_key, bob_key,
            "distinct users must not share a namespace"
        );
    }

    /// Trusted internal/operator credentials keep the original project key so
    /// deployment probes and API-key callers address projects directly.
    #[test]
    fn trusted_callers_keep_the_logical_project_key() {
        let operator = AuthContext { user_id: None };
        assert_eq!(scoped_project_id(&operator, "acme-api"), "acme-api");
    }

    #[test]
    fn security_scheme_pairs_the_scope_with_offline_access() {
        let scheme = security(ARCHITECTURE_SCOPE);
        assert_eq!(scheme[0]["scopes"][0], ARCHITECTURE_SCOPE);
        assert_eq!(scheme[0]["scopes"][1], OFFLINE_SCOPE);
    }

    /// The RFC 9728 challenge must name the resource metadata document, the
    /// scope being demanded, and offline_access, or clients cannot discover
    /// where to authenticate.
    #[test]
    fn oauth_challenge_carries_resource_metadata_and_scope() {
        let response = oauth_required_result(
            Value::from(1),
            ARCHITECTURE_SCOPE,
        ...
        assert_eq!(
            response.status(),
            StatusCode::OK,
            "MCP errors ride inside the JSON-RPC result"
        );
        ...
        assert!(challenge.contains(ARCHITECTURE_SCOPE));
        assert!(challenge.contains(OFFLINE_SCOPE));
        assert!(challenge.contains("insufficient_scope"));
    }

    #[test]
    fn resource_and_issuer_drop_trailing_slashes() {
        assert!(!mcp_resource().ends_with('/'));
        assert!(!oauth_issuer().ends_with('/'));
    }
