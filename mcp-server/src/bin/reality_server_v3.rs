...
struct ImpactRequest {
    path: Option<String>, file: String, #[serde(default = "default_line")] line: u32,
    change_type: Option<String>, repo_name: Option<String>, project_id: Option<String>,
}
fn default_line() -> u32 { 1 }
#[derive(Debug, Deserialize)]
struct OtlpRequest {
    raw_spans: Option<String>, otlp_json: Option<String>, payload: Option<Value>,
    repo_name: Option<String>, project_id: Option<String>,
}
#[derive(Debug, Deserialize)]
struct DiffRequest { from_snapshot: String, to_snapshot: String, project_id: Option<String>, repo_name: Option<String> }

fn internal<E: std::fmt::Display>(e: E) -> (StatusCode, String) {
    (StatusCode::INTERNAL_SERVER_ERROR, e.to_string())
}
fn key(repo: Option<&str>, project: Option<&str>) -> String {
    project.filter(|s| !s.is_empty()).or_else(|| repo.filter(|s| !s.is_empty())).unwrap_or("default").to_string()
}
...
async fn load_session(state: &AppState, project_key: &str) -> Session {
    ...
    let path = session_file(state, project_key);
...
async fn persist_snapshot(state: &AppState, s: &Session) -> anyhow::Result<Option<String>> {
    ...
    let snap = ArchitectureSnapshot {
        id: id.clone(), project_key: s.project_key.clone(), created_at: chrono::Utc::now().to_rfc3339(), report, graph,
    };
...
async fn read_snapshot(state: &AppState, project_key: &str, id: &str) -> anyhow::Result<ArchitectureSnapshot> {
    let bytes = tokio::fs::read(snapshot_dir(state, project_key).join(format!("{}.bin", safe_key(id)))).await?;
...
async fn auth(State(state): State<AppState>, headers: HeaderMap, request: axum::http::Request<axum::body::Body>, next: Next) -> Result<Response, (StatusCode, String)> {
    if let Some(expected) = &state.api_key {
        if extract_key(&headers).as_deref() != Some(expected.as_str()) { return Err((StatusCode::UNAUTHORIZED, "Missing or invalid CKB API key".into())); }
    }
...
fn discover(root: &str) -> anyhow::Result<Vec<PathBuf>> {
    ...
    while let Some(dir) = stack.pop() {
        for entry in std::fs::read_dir(&dir)? {
            let entry = entry?; let p = entry.path(); let name = p.file_name().and_then(|v| v.to_str()).unwrap_or("");
            if p.is_dir() { if !matches!(name, ".git"|"node_modules"|"target"|"dist"|"build"|".next"|"vendor") { stack.push(p) } }
            else if supported(&p) { out.push(p) }
        }
    }
    Ok(out)
}
...
fn external_dependencies(analyses: &[FileAnalysis]) -> Vec<String> {
    ...
    for a in analyses { for i in &a.imports { if i.source.starts_with('.') || i.source.starts_with('/') || i.source.is_empty(){continue} let n=if let Some(s)=i.source.strip_prefix('@'){let mut p=s.split('/');match(p.next(),p.next()){(Some(a),Some(b))=>format!("@{}/{}",a,b),_=>i.source.clone()}}else{i.source.split('/').next().unwrap_or(&i.source).to_string()}; d.insert(n); } }
    d.into_iter().collect()
}
...
async fn health(State(state):State<AppState>)->Json<Value>{
    Json(json!({"status":"healthy","service":"ckb-reality-server-v3","realityApi":"v3","graphPersistence":"durable-bincode-snapshots","dataDir":state.data_dir.to_string_lossy(),"evidencePolicy":"static-runtime-predicted-separated"}))
}
async fn scan(State(state):State<AppState>,Json(req):Json<ScanRequest>)->Result<Json<Value>,(StatusCode,String)>{
    let project_key=key(req.repo_name.as_deref(),req.project_id.as_deref()); let s=load_session(&state,&project_key).await; let (graph,report)=build_graph(&req.path).await.map_err(internal)?;
    *s.graph.write().await=Some(graph); *s.report.write().await=Some(report.clone()); *s.repo_path.write().await=Some(req.path); s.runtime_nodes.write().await.clear(); s.runtime_edges.write().await.clear();
    persist_session(&state,&s).await.map_err(internal)?; let snapshot_id=persist_snapshot(&state,&s).await.map_err(internal)?;
    Ok(Json(json!({"status":"success","projectKey":project_key,"filesProcessed":report.files_processed,"nodes":report.nodes,"edges":report.edges,"violationsFound":report.drift.len(),"snapshotId":snapshot_id})))
}
async fn report(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<Json<ScanReport>,(StatusCode,String)>{
    let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;
    s.report.read().await.clone().map(Json).ok_or((StatusCode::NOT_FOUND,"No scan has been run for this session".into()))
}
async fn impact(State(state):State<AppState>,Json(req):Json<ImpactRequest>)->Result<Json<Value>,(StatusCode,String)>{
    let s=load_session(&state,&key(req.repo_name.as_deref(),req.project_id.as_deref())).await;
    if s.graph.read().await.is_none(){let path=req.path.as_deref().ok_or((StatusCode::PRECONDITION_REQUIRED,"Scan first or provide path".into()))?;let(g,r)=build_graph(path).await.map_err(internal)?;*s.graph.write().await=Some(g);*s.report.write().await=Some(r);*s.repo_path.write().await=Some(path.into());persist_session(&state,&s).await.map_err(internal)?;}
    ...
    Ok(Json(json!({"kind":"predicted","confidencePolicy":"derived-per-path","assumptions":["Current persisted graph is the baseline"],"evidence":[{"source":"ast-graph","ref":format!("{}:{}",req.file,req.line)}],"result":result})))
}
...
async fn graph_api(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<Json<Value>,(StatusCode,String)>{
    let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let g=s.graph.read().await;let g=g.as_ref().ok_or((StatusCode::PRECONDITION_REQUIRED,"No scan has been run for this session".into()))?;let rn=s.runtime_nodes.read().await.clone();let re=s.runtime_edges.read().await.clone();
...
async fn otlp(State(state):State<AppState>,Json(req):Json<OtlpRequest>)->Result<Json<Value>,(StatusCode,String)>{let s=load_session(&state,&key(req.repo_name.as_deref(),req.project_id.as_deref())).await;let raw=if let Some(v)=req.raw_spans.or(req.otlp_json){v}else if let Some(v)=req.payload{serde_json::to_string(&v).map_err(internal)?}else{return Err((StatusCode::BAD_REQUEST,"Provide raw_spans, otlp_json, or payload".into()))};let n=OtlpReceiver::ingest_spans(&raw).map_err(internal)?;let e=edge_observations(&raw).map_err(internal)?;let summary=OtlpReceiver::summarize(&n);merge_nodes(&mut s.runtime_nodes.write().await,n);merge_edges(&mut s.runtime_edges.write().await,e);persist_session(&state,&s).await.map_err(internal)?;Ok(Json(json!({"status":"observed","kind":"runtime","spansIngested":summary.spans_ingested,"nodesUpdated":summary.nodes_updated,"errorSpans":summary.error_spans,"hotpathNodes":summary.hotpath_nodes,"runtimeEdges":s.runtime_edges.read().await.len(),"persistence":"durable","evidence":[{"source":"otlp","ref":"ingested-payload"}]})))}
async fn runtime(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Json<Value>{let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let n=s.runtime_nodes.read().await.iter().map(|(id,m)|json!({"id":id.0,"invocationCount":m.execution_count,"avgLatencyMs":m.avg_latency_ms,"errorRate":m.error_rate,"isHotpath":m.is_hotpath,"kind":"runtime","evidence":[{"source":"otlp","ref":id.0}]})).collect::<Vec<_>>();let e=s.runtime_edges.read().await.values().map(|r|json!({"source":r.source,"target":r.target,"traceId":r.trace_id,"invocationCount":r.invocation_count,"avgLatencyMs":r.avg_latency_ms(),"errorRate":r.error_rate(),"lastSeenUnixNano":r.last_seen_unix_nano,"kind":"runtime","evidence":[{"source":"otlp-parent-child","ref":format!("{}->{}",r.source,r.target)}]})).collect::<Vec<_>>();Json(json!({"observed":!n.is_empty()||!e.is_empty(),"nodes":n,"edges":e,"persistence":"durable"}))}
async fn traces(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Json<Value>{let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let e=s.runtime_edges.read().await;let mut t:HashMap<String,Vec<Value>>=HashMap::new();for x in e.values(){t.entry(x.trace_id.clone()).or_default().push(json!({"source":x.source,"target":x.target,"invocationCount":x.invocation_count,"avgLatencyMs":x.avg_latency_ms(),"errorRate":x.error_rate(),"lastSeenUnixNano":x.last_seen_unix_nano}))}Json(json!({"kind":"runtime","observed":!e.is_empty(),"traces":t}))}
async fn source(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<Json<Value>,(StatusCode,String)>{let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let id=q.get("node_id").ok_or((StatusCode::BAD_REQUEST,"node_id is required".into()))?;let g=s.graph.read().await;let g=g.as_ref().ok_or((StatusCode::PRECONDITION_REQUIRED,"No scan".into()))?;let n=g.nodes().into_iter().find(|n|n.id.0==*id).ok_or((StatusCode::NOT_FOUND,"Node not found".into()))?;Ok(Json(json!({"id":n.id.0,"name":n.name,"kind":kind(n.kind),"path":n.path,"line":n.line,"column":n.column,"span":{"startLine":n.metadata.get("start_line"),"startColumn":n.metadata.get("start_column"),"endLine":n.metadata.get("end_line"),"endColumn":n.metadata.get("end_column"),"byteStart":n.metadata.get("byte_start"),"byteEnd":n.metadata.get("byte_end")},"kindOfEvidence":"static","confidence":1.0,"evidence":[{"source":"tree-sitter-ast","ref":n.id.0}]})))}
async fn history(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<Json<Value>,(StatusCode,String)>{let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let p=s.repo_path.read().await.clone().or_else(||q.get("path").cloned()).ok_or((StatusCode::PRECONDITION_REQUIRED,"Scan first or provide path".into()))?;let max=q.get("max_commits").and_then(|v|v.parse::<usize>().ok()).unwrap_or(50).min(500);let t=GitDriftAnalyzer::build_timeline(&p,max).map_err(internal)?;Ok(Json(json!({"kind":"static","source":"git","timeline":t,"evidence":[{"source":"git-history","ref":p}]})))}
async fn snapshots(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<Json<Value>,(StatusCode,String)>{let project_key=key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str));let dir=snapshot_dir(&state,&project_key);let mut out=Vec::new();if let Ok(mut rd)=tokio::fs::read_dir(dir).await{while let Ok(Some(e))=rd.next_entry().await{if let Ok(bytes)=tokio::fs::read(e.path()).await{if let Ok(s)=bincode::deserialize::<ArchitectureSnapshot>(&bytes){out.push(json!({"id":s.id,"createdAt":s.created_at,"nodes":s.report.nodes,"edges":s.report.edges,"filesProcessed":s.report.files_processed,"violations":s.report.drift.len(),"durationMs":s.report.duration_ms}))}}}}out.sort_by(|a,b|a.get("createdAt").and_then(Value::as_str).cmp(&b.get("createdAt").and_then(Value::as_str)));Ok(Json(json!({"projectKey":project_key,"snapshots":out,"source":"ckb-persistent-snapshots"})))}
async fn diff(State(state):State<AppState>,Json(req):Json<DiffRequest>)->Result<Json<Value>,(StatusCode,String)>{let project_key=key(req.repo_name.as_deref(),req.project_id.as_deref());let a=read_snapshot(&state,&project_key,&req.from_snapshot).await.map_err(internal)?;let b=read_snapshot(&state,&project_key,&req.to_snapshot).await.map_err(internal)?;let an:HashSet<String>=a.graph.nodes().into_iter().map(|n|n.id.0.clone()).collect();let bn:HashSet<String>=b.graph.nodes().into_iter().map(|n|n.id.0.clone()).collect();let ae:HashSet<String>=a.graph.edges().into_iter().map(|e|format!("{}->{}/{}",e.from.0,e.to.0,kind(e.kind))).collect();let be:HashSet<String>=b.graph.edges().into_iter().map(|e|format!("{}->{}/{}",e.from.0,e.to.0,kind(e.kind))).collect();let added_nodes=bn.difference(&an).cloned().collect::<Vec<_>>();let removed_nodes=an.difference(&bn).cloned().collect::<Vec<_>>();let added_edges=be.difference(&ae).cloned().collect::<Vec<_>>();let removed_edges=ae.difference(&be).cloned().collect::<Vec<_>>();Ok(Json(json!({"kind":"static","source":"persistent-snapshot-diff","from":req.from_snapshot,"to":req.to_snapshot,"addedNodes":added_nodes,"removedNodes":removed_nodes,"addedEdges":added_edges,"removedEdges":removed_edges,"summary":{"nodeDelta":bn.len() as i64-an.len() as i64,"edgeDelta":be.len() as i64-ae.len() as i64,"fromViolations":a.report.drift.len(),"toViolations":b.report.drift.len()},"evidence":[{"source":"ckb-snapshot","ref":a.id},{"source":"ckb-snapshot","ref":b.id}]})))}
async fn test_gaps(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<Json<Value>,(StatusCode,String)>{let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let g=s.graph.read().await;let g=g.as_ref().ok_or((StatusCode::PRECONDITION_REQUIRED,"No scan".into()))?;Ok(Json(serde_json::to_value(TestCoverageAnalyzer::analyze_gaps(g).map_err(internal)?).map_err(internal)?))}
async fn rules(State(state):State<AppState>,Query(q):Query<HashMap<String,String>>)->Result<String,(StatusCode,String)>{let s=load_session(&state,&key(q.get("repo").map(String::as_str),q.get("project_id").map(String::as_str))).await;let g=s.graph.read().await;let g=g.as_ref().ok_or((StatusCode::PRECONDITION_REQUIRED,"No scan".into()))?;ArchitectureAnalyzer::new().generate_ai_guidelines(g).map_err(internal)}
...
async fn main()->anyhow::Result<()> {
    ...
    let state=AppState{sessions:Arc::new(RwLock::new(HashMap::new())),api_key:std::env::var("CKB_API_KEY").ok().filter(|v|!v.is_empty()).map(Arc::new),data_dir:Arc::new(data_dir)};
    if state.api_key.is_none(){warn!("CKB_API_KEY is not configured; Reality API is unauthenticated")}
    let protected=Router::new()
        .route("/api/v1/scan",post(scan)).route("/api/v1/report",get(report)).route("/api/v1/impact",post(impact)).route("/api/v1/otlp",post(otlp))
        .route("/api/v1/drift-timeline",get(history)).route("/api/v1/test-gaps",get(test_gaps)).route("/api/v1/rules",get(rules))
        .route("/api/v1/intelligence/graph",get(graph_api)).route("/api/v1/intelligence/source",get(source)).route("/api/v1/intelligence/runtime",get(runtime))
        .route("/api/v1/intelligence/traces",get(traces)).route("/api/v1/intelligence/impact",post(impact)).route("/api/v1/intelligence/telemetry/otlp",post(otlp))
        .route("/api/v1/intelligence/history",get(history)).route("/api/v1/intelligence/snapshots",get(snapshots)).route("/api/v1/intelligence/diff",post(diff))
        .route_layer(middleware::from_fn_with_state(state.clone(),auth));
    let cors=match std::env::var("CKB_ALLOWED_ORIGIN"){Ok(v)if v=="*"=>CorsLayer::new().allow_origin(Any).allow_methods(Any).allow_headers(Any),Ok(v)=>CorsLayer::new().allow_origin(v.parse::<axum::http::HeaderValue>()?).allow_methods(Any).allow_headers(Any),Err(_)=>CorsLayer::new()};
    let app=Router::new().route("/health",get(health)).merge(protected).layer(cors).with_state(state);
    let port=std::env::var("PORT").ok().and_then(|v|v.parse().ok()).unwrap_or(3000);let all=std::env::var("CKB_BIND_ALL").map(|v|v=="1"||v.eq_ignore_ascii_case("true")).unwrap_or(false);let host=if all{[0,0,0,0]}else{[127,0,0,1]};let addr=std::net::SocketAddr::from((host,port));info!("CKB Reality API v3 listening on {}",addr);let l=tokio::net::TcpListener::bind(addr).await?;axum::serve(l,app).await?;Ok(())
}
