...
fn architecture_evidence(violations: &[ckb_core::DriftViolation]) -> ArchitectureEvidence {
    ...
    // Architecture evidence is intentionally conservative. Critical breaches
    // are never normalized away by a large clean codebase. Lower severities
    // contribute progressively smaller penalties, capped at 100 points.
...
mod tests {
    use super::*;
    use ckb_core::{DriftKind, DriftViolation};

    fn violation(severity: Severity) -> DriftViolation {
        DriftViolation {
            id: "test".to_string(),
            kind: DriftKind::LayerBreach,
            from: "a".to_string(),
            to: "b".to_string(),
